...
<c:set var="fundingDTO" value="${funding}"/>
<!DOCTYPE html>
<html>
<head>
...
<div>
	...
	<div class="header3">
		...
		<div class="header3_summary">
		    <strong class="dday">D-<c:out value="${fundingDDAY}" /></strong>
			...
			<div class="content_reply">
				...
				<ul>
					<li>
						...
						<div class="reply_date">${boardFBuyDate }</div>
						<div class="reply_user">${userId}님 ${boardFPickAmount}원 참여</div>
					</li>
			    </ul>
			</div>
...
$(document).ready(function() {
    // fundingDTO.boardFSeq는 출력하고자 하는 게시물의 시퀀스입니다.
    var boardFSeq = fundingDTO.boardFSeq;

    $.ajax({
        url: "/Hangeulum/funding/fun_getBoard/"
        type: "GET",
        dataType: "json",
        success: function(s) {
            var fundingDTO = data;
            ...
            var dDay = "D-" + diffInDays;

            var html = '';
            html += '<div class="header3">';
            html += '<div class="header3_img">';
            html += '<img src="/Hangeulum/storage/' + fundingDTO.boardFThumbnail + '" width="668px" height="401px">';
            html += '</div>'; // header3 이미지 끝
            html += '<div class="header3_summary">';
            html += '<strong class="dday">' + dDay + '</strong>';
            html += '<h3 class="title">' + fundingDTO.boardFSubject + '</h3>';
            html += '<div class="graph">';
            html += '<div class="rate">';
            html += '<strong class="per"></strong>';
            html += '</div>'; // rate
            html += '<div class="bar_main">';
            html += '<span class="bar" style="width: 100%;"></span>';
            html += '</div>'; // bar_main
            html += '<div class="credit_goal">';
            html += '<span class="credit">' + fundingDTO.boardFGoalAmount + '원 목표</span>';
            html += '</div>'; // credit_main
            html += '<div class="credit_much">';
            html += '<span class="much">2,014,000원</span>';
            html += '</div>'; // credit_much
            html += '</div>'; // graph 끝
            html += '</div>'; // header3_summary 끝
            html += '</div>'; // header3 끝

            $("#fundingDetail").html(html);
        },
        error: function() {
